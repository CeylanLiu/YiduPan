	...
	public UploadResultDto uploadFile(SessionWebUserDto webUserDto, String fileId, MultipartFile file, String fileName,
		...
		try {
			...
			if (chunkIndex == 0) {
				...
				// 暂存临时目录 e:/javacode/myworkspace/temp/
			...
			String month = DateUtil.format(new Date(), DateTimePatternEnum.YYYYMM.getPattern());// 月份
			String fileSuffix = StringTools.getFileSuffix(fileName);// 文件后缀
			// 真实文件名
...
//    @Transactional
	...
	public void transferFile(String fileId, SessionWebUserDto webUserDto) {
		...
		try {
			...
			//目标目录 e:/javacode/myworkspace/temp/ + /file + /{month}
			...
			System.out.println(new File(targetFilePath).length());
